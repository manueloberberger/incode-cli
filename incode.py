...
def setup_auth(force_interactive: bool = False) -> Tuple[str, str, Optional[str], Optional[List[str]]]:
    ...
    # 1. No users -> Fresh Install Menu
    if not users:
        # Check if we should show the welcome menu or just prompt (default behavior was just prompt)
        # We now offer Import
        
        # Simple interactive menu for fresh start
        ...
        while True:
            ...
            if selected == "new":
                ...
                console.print()
                console.print(Align.center("[dim]Bitte gib den Pfad zur Backup-Datei (.json) an.[/dim]"))
                fpath = centered_input("[bold green]Dateipfad >[/bold green] ")
                
                if fpath and fpath.strip():
                    # Try import
                    console.print()
                    with console.status("[bold blue]Importiere Daten...[/bold blue]"):
                        success = import_data(fpath.strip())
                    
                    if success:
                        wait_for_return()
                        # Reload credentials and see if we have users now
                        creds_data = load_credentials()
                        users = creds_data.get('users', [])
                        
                        if users:
                            # If single user, auto-login (recursion)
                            # Or if multiple, show menu (recursion)
                            return setup_auth(force_interactive=force_interactive)
                        else:
                             console.print(Align.center("[yellow]Import erfolgreich, aber keine Benutzer gefunden?[/yellow]"))
                             wait_for_return()
                    else:
                        wait_for_return()
...
def show_help() -> None:
    ...
    help_text = [
        ...
        ("./incode --export [FILE]", "Exportiert alle Daten (Backup). Optional Dateiname."),
        ("./incode --import <FILE>", "Importiert Daten aus einer Sicherungsdatei."),
...
if __name__ == "__main__":
    try:
        ...
        if len(sys.argv) > 1 and "install-service" in sys.argv:
        ...
        elif "--export" in sys.argv or "--import" in sys.argv:
            ...
            if "--export" in sys.argv:
                try:
                    ...
                    if idx + 1 < len(sys.argv) and not sys.argv[idx+1].startswith("-"):
                        ...
                        filepath = f"incode_backup_{timestamp}.json"
                    
                    console.print(Align.center(f"[bold blue]Exportiere Daten nach {filepath}...[/bold blue]"))
                    if export_data(filepath):
                        console.print(Align.center("[bold green]Export erfolgreich abgeschlossen![/bold green]"))
                    sys.exit(0)
                except Exception as e:
                    console.print(f"[red]Fehler bei Export: {e}[/red]")
                    sys.exit(1)
            ...
            if "--import" in sys.argv:
                try:
                    ...
                    if idx + 1 < len(sys.argv):
                        filepath = sys.argv[idx + 1]
                        console.print(Align.center(f"[bold blue]Importiere Daten aus {filepath}...[/bold blue]"))
                        if import_data(filepath):
                             console.print(Align.center("[bold green]Import erfolgreich![/bold green]"))
                        sys.exit(0)
                    else:
                        console.print("[red]Fehler: Bitte Dateipfad für Import angeben.[/red]")
                        sys.exit(1)
                except Exception as e:
                    console.print(f"[red]Fehler bei Import: {e}[/red]")
                    sys.exit(1)
        else:
